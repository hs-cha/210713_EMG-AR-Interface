...
def run(addresses):
    ...
    tasks = asyncio.gather(*(connect_to_imu_device(address) for address in addresses))

    loop.run_until_complete(tasks)


async def connect_to_imu_device(address):
...
# loop = asyncio.get_event_loop()
# loop.set_debug(True)
# loop.run_until_complete(run(address1, False))
# loop.run_until_complete(run(address2, False))
